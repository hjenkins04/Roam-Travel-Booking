...
import { DisplayPurchasePassenger, getFlightIdString , getLayoverSummary, formatTimeMinutes } from "@/models";
    ...
    <div className="flex flex-col max-md:ml-0 max-md:w-full">
      <div className="flex flex-col grow max-md:max-w-full">
        <div className="flex flex-col items-start pr-6 pl-6 w-full max-md:px-5 max-md:max-w-full">
          ...
          <time className="text-lg font-semibold text-slate-500">{formattedDepartureDate}</time>
          <p className="text-base text-black">Departing {departingFlight.departure_airport.iata_code}</p>
          ...
          <div className="flex gap-2 items-start self-stretch mt-2 max-md:ml-2">
            ...
            <div className="flex flex-col self-stretch text-xs">
              ...
              <p className="self-start text-black">{departingFlight.baggage_allowance}</p>
            </div>
            ...
            <div className="flex flex-col text-base text-right min-h-[74px] text-slate-800">
              ...
              {departingFlight.layover && <p className="mt-1 text-black">{getLayoverSummary(departingFlight)}</p>}
            </div>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="flex flex-col max-md:ml-0 max-md:w-full">
      <div className="flex flex-col grow max-md:max-w-full">
        <div className="flex flex-col items-start pr-6 pl-6 w-full max-md:px-5 max-md:max-w-full">
          {/* Flight date and departure information */}
          <time className="text-lg font-semibold text-slate-500">{formattedReturnDate}</time>
          <p className="text-base text-black">Departing {returningFlight.departure_airport.iata_code}</p>
          {/* Flight details container */}
          <div className="flex gap-2 items-start self-stretch mt-2 max-md:ml-2">
            ...
            <div className="flex flex-col self-stretch text-xs">
              <div className="flex flex-col text-base min-h-[48px]">
                ...
                <p className="mt-1 text-black">{getFlightIdString(returningFlight)}</p>
              </div>
              <p className="text-black max-md:mr-2.5">{returnSeat}</p>
              <p className="self-start text-black">{returningFlight.baggage_allowance}</p>
            </div>
            {/* Flight duration, times, and layover information */}
            <div className="flex flex-col text-base text-right min-h-[74px] text-slate-800">
              <p>{formatTimeMinutes(returningFlight.flight_time_minutes)}</p>
              <p className="mt-1">
                {returningFlight.departure_time} - {returningFlight.arrival_time}
              </p>
              {returningFlight.layover && <p className="mt-1 text-black">{getLayoverSummary(returningFlight)}</p>}
            </div>
          </div>
        </div>
      </div>
